...
const applyReferralCode = async (newUserId, referralCode) => {
  try {
    ...
    await Notification.create(
      newUserId,
      '🎁 Welcome Bonus!',
      'You received 50 Valiant Bucks from using a referral code!',
      'referral_bonus'
    );
    ...
    await Notification.create(
      referrer.id,
      '👋 New Referral!',
      'Someone used your referral code! You\'ll earn 100 Valiant Bucks when they place their first bet.',
      'referral_pending'
    );
...
const completeReferral = async (userId) => {
  try {
    ...
    await Notification.create(
      referral.referrer_id,
      '💰 Referral Complete!',
      `Your referral placed their first bet! You earned ${referral.referrer_reward} Valiant Bucks!`,
      'referral_complete'
    );
